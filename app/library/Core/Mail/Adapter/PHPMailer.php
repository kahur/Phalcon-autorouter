...
//        }
     ...
     * @param string $name
     * @throws \Core\Mail\Adapter\Exception
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
            ...
            if ($this->exceptions) {
                throw new \Core\Mail\Adapter\Exception('Invalid recipient array: ' . $kind);
            }
            return false;
        }
        ...
        if (!$this->validateAddress($address)) {
            ...
            if ($this->exceptions) {
                throw new \Core\Mail\Adapter\Exception($this->lang('invalid_address') . ': ' . $address);
            }
            return false;
        }
        if ($kind != 'Reply-To') {
    ...
    public function send()
    {
        ...
        } catch (\Core\Mail\Adapter\Exception $exc) {
            $this->mailHeader = '';
    ...
    public function preSend()
    {
        try {
            ...
            if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
                throw new \Core\Mail\Adapter\Exception($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new \Core\Mail\Adapter\Exception($this->lang('empty_message'), self::STOP_CRITICAL);
            }
        ...
        if ($this->SingleTo === true) {
            foreach ($this->SingleToArray as $toAddr) {
                if (!@$mail = popen($sendmail, 'w')) {
                    throw new \Core\Mail\Adapter\Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new \Core\Mail\Adapter\Exception($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect()) {
            throw new \Core\Mail\Adapter\Exception($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        $smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
        if (!$this->smtp->mail($smtp_from)) {
            $this->setError($this->lang('from_failed') . $smtp_from . ' : ' . implode(',', $this->smtp->getError()));
            throw new \Core\Mail\Adapter\Exception($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new \Core\Mail\Adapter\Exception($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) { // Create error message for any bad addresses
            throw new \Core\Mail\Adapter\Exception(
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new \Core\Mail\Adapter\Exception($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new \Core\Mail\Adapter\Exception($this->lang('authenticate'));
                        }
                    }
                    return true;
                } catch (\Core\Mail\Adapter\Exception $exc) {
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new \Core\Mail\Adapter\Exception($this->lang('signing') . ' OpenSSL extension missing.');
                }
                ...
                if (@openssl_pkcs7_sign(
                    ...
                    @unlink($file);
                    @unlink($signed);
                    throw new \Core\Mail\Adapter\Exception($this->lang('signing') . openssl_error_string());
                }
            } catch (\Core\Mail\Adapter\Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new \Core\Mail\Adapter\Exception($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!is_readable($path)) {
                throw new \Core\Mail\Adapter\Exception($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
    ...
    public function set($name, $value = '')
    {
        try {
            if (isset($this->$name)) {
                ...
                throw new \Core\Mail\Adapter\Exception($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            }
        } catch (Exception $exc) {
            $this->setError($exc->getMessage());
